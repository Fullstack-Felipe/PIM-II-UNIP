...
int autenticarUsuario() {
    ...
    while (1) {
        ...
        printf("\n---------------------------------------------\n");
        printf(" MUSEU DA HOMENAGEM MULTICULTURAL\n");
        printf("---------------------------------------------\n\n");
...
void enviarIngressosPorEmail(const struct Ingresso ingressos[], int numIngressos) {
    ...
    printf("\n Enviando ingressos por e-mail para: %s\n", email);
...
void validarIngresso() {
   ...
   int searchTicketNumber; // Número de ingresso a ser pesquisado
    printf(" Digite o número de venda impresso no ingresso: ");
    ...
    // Ler o arquivo linha por linha
        while (fgets(line, MAX_LINE_LENGTH, file) != NULL) {
            ...
            // Pula o primeiro token (primeira coluna)
            token = strtok(NULL, ";");
